...
export const modifyBudgetSchema = budgetSchema.pick({
  category: true,
  period: true,
});

export const updateBudgetTool = createTool({
  id: "update-budget-tool",

  description: `
Updates a spending budget for one expense category.

Use this whenever the user wants to:

- update a budget
- modify a monthly budget
`,
  ...
  execute: async (input, context) => {
    ...
    process.stdout.write(
      `${new Date().toISOString()} - Update budget initiated with input: ${JSON.stringify(input)}\n`,
    );

    const { searchCriteria, updateData } = input;
    try {
      if (!userId) {
        return {
          isSuccessful: false,
      ...
      const budget = await budgetService.updateBudget({
        ...updateData,
        id: foundBudget.id,
        limitAmount: updateData?.limitAmount?.toString(),
